 ...
 */
function resolveSafePath(target, baseDir) {
    if (!target) {
        return baseDir;
    }
    // Resolve to absolute path
    const resolved = node_path_1.default.isAbsolute(target) ? node_path_1.default.normalize(target) : node_path_1.default.resolve(baseDir, target);
    ...
    const inputParts = target.split(node_path_1.default.sep);
    const resolvedParts = normalized.split(node_path_1.default.sep);
    ...
    if (process.platform === 'win32') {
        ...
        const baseDrive = baseDir.split(node_path_1.default.sep)[0];
        const resolvedDrive = normalized.split(node_path_1.default.sep)[0];
